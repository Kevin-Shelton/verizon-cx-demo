...
export default function Journey() {
  ...
  const personaDetails: Record<string, { painPoints: string[]; interactionNeeds: string[]; narrative: string }> = {
    carlos: {
      narrative: "Carlos runs a small retail shop in Los Angeles, California and needs to expand his business phone lines. As a Mexican immigrant, he prefers communicating in Mexican Spanish. During **Outbound Prospecting**, he receives an email in Mexican Spanish from a Verizon rep and calls the IVR system to learn more, navigating the menu in his dialect. In the **Selling** stage, a Verizon field sales rep visits his store with a tablet showing the in-field app, walking him through device options, pricing, and the number porting process in Mexican Spanish. Carlos uses the website to compare plans and builds a quote with the rep. He receives a translated contract document via email, reviews it, and signs electronically. During **Service & Post Sales**, he receives SMS updates about his port status, device shipment, and account activation. He accesses the customer portal in Mexican Spanish to track his order, manage his account, and contact support via live chat when he has questions about his first bill.",
      painPoints: [
    ...
    maria: {
      narrative: "María manages field service technicians across Florida and needs to swap devices for her team. Originally from Puerto Rico, she prefers communicating in Caribbean Spanish. During **Service & Post Sales**, she visits a Verizon retail store in Miami where an associate uses the in-store tablet to check her account and promo eligibility in Caribbean Spanish. She browses the website to review trade-in options and device upgrade paths. When she needs technical support, she uses the live chat feature on the website, communicating in Puerto Rican Spanish with real-time translation. She receives a translated promo redemption document via email explaining the trade-in process and eligibility requirements. When she calls the IVR to check on her device swap status, she navigates the menu in her dialect and gets transferred to a Spanish-speaking support specialist. A field sales rep follows up with her team on-site, using the mobile app to demonstrate new device features in Caribbean Spanish and providing training materials in her preferred dialect. She receives SMS notifications when devices are ready for pickup.",
      painPoints: [
    ...
    lucia: {
      narrative: "Lucía administers a healthcare clinic in Queens, New York and receives a fraud alert via email in Colombian Spanish. Originally from Colombia, she prefers communicating in Latin American Spanish. During **Churn Mitigation**, she immediately calls the IVR system, which recognizes her dialect and routes her to security support to address the fraud alert and prevent potential account disconnection. She visits the Verizon website to open a support case, navigating the authenticated portal in Latin American Spanish. During **Service & Post Sales**, a field sales rep visits her clinic with a tablet to review her account security settings, provide HIPAA-compliant documentation in Spanish, and discuss additional security features. She receives a translated incident report document via email, along with step-by-step instructions for securing her account. Throughout the resolution process, she uses the live chat feature to ask questions and receives SMS updates in Colombian Spanish about her case status. She also accesses translated training documents via the portal to educate her clinic staff on security best practices.",
      painPoints: [
    ...
    diego: {
      narrative: "Diego manages construction projects across the US Southwest and needs to provision 50 devices for his crew. During **Outbound Prospecting**, he receives an email in US Spanish about business solutions and browses the Verizon Business website to explore bulk device options. He calls the IVR to speak with a business sales specialist, navigating the menu in Spanish. In the **Selling** stage, a Verizon field sales rep visits his job site with a tablet, using the in-field app to configure a fleet management solution in US Spanish. They review device options, pricing tiers, and service plans on the tablet. Diego receives a detailed quote document via email, translated into his dialect with flexible payment terms. He reviews the contract on the website portal and signs electronically. During **Service & Post Sales**, he visits a Verizon store to pick up devices, where associates use in-store tablets to activate and configure each device in Spanish. Throughout the deployment, he receives SMS updates about device activation and account setup. He accesses the fleet management portal on the website in US Spanish to monitor usage, manage billing, add new lines, and request support via live chat when needed.",
      painPoints: [
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-7xl w-full max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 h-[calc(90vh-88px)] overflow-y-auto visual-flow-container" style={{ scrollbarWidth: 'auto', scrollbarColor: '#3b82f6 #e5e7eb' }}>
              ...
              {selectedPersona !== 'all' && (() => {
                ...
                if (!persona || !details) return null;
                return (
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-lg p-5 mb-4">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-3xl shadow-lg border-2 border-blue-400">
                        {persona.avatar}
                      </div>
                      <div className="flex-1">
                        <h3 className="text-xl font-bold text-gray-900 mb-1">{persona.name} - {persona.role}</h3>
                        <div className="flex items-center gap-2 mb-2">
                          <svg className="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" />
                          </svg>
                          <span className="text-sm font-semibold text-blue-700">{persona.dialectLabel}</span>
                          <span className="text-xs text-gray-500 ml-2">• {fullCoverageActivities.filter(a => a.personas?.includes(persona.id)).length} touchpoints</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-lg p-4 mb-3 border border-blue-200">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed">{details.narrative}</p>
                    </div>
              ...
              <div className="flex items-start gap-6 overflow-x-auto pb-6 visual-flow-horizontal" style={{ minWidth: 'max-content', scrollbarWidth: 'auto', scrollbarColor: '#3b82f6 #e5e7eb' }}>
                ...
                {journeyData.stages.map((stage) => {
                  return stage.activities.map((activity) => {
                    ...
                    const stageColors: Record<string, { bg: string; border: string; text: string; headerBg: string; headerText: string }> = {
                      'outbound': { bg: 'bg-orange-50', border: 'border-orange-500', text: 'text-orange-900', headerBg: 'bg-orange-600', headerText: 'text-white' },
                      'qualification': { bg: 'bg-red-50', border: 'border-red-500', text: 'text-red-900', headerBg: 'bg-red-600', headerText: 'text-white' },
                      'proposal': { bg: 'bg-red-50', border: 'border-red-500', text: 'text-red-900', headerBg: 'bg-red-600', headerText: 'text-white' },
                      'negotiation': { bg: 'bg-yellow-50', border: 'border-yellow-500', text: 'text-yellow-900', headerBg: 'bg-yellow-600', headerText: 'text-white' },
                      'onboarding': { bg: 'bg-red-50', border: 'border-red-500', text: 'text-red-900', headerBg: 'bg-red-600', headerText: 'text-white' },
                      'support': { bg: 'bg-green-50', border: 'border-green-500', text: 'text-green-900', headerBg: 'bg-green-600', headerText: 'text-white' },
                      'expansion': { bg: 'bg-green-50', border: 'border-green-500', text: 'text-green-900', headerBg: 'bg-green-600', headerText: 'text-white' }
                    };

                    const colors = stageColors[stage.id] || { bg: 'bg-gray-50', border: 'border-gray-400', text: 'text-gray-700', headerBg: 'bg-gray-700', headerText: 'text-white' };
                    ...
                    return (
                      <div key={`${stage.id}-${activity.id}`} className="relative flex items-center gap-6">
                        ...
                        <div className={`flex flex-col ${colors.bg} border-2 ${colors.border} rounded-lg shadow-lg min-h-[400px] w-64`}>
                          {/* Header */}
                          <div className={`${colors.headerBg} ${colors.headerText} px-4 py-3 rounded-t-lg`}>
