...
export default function LaunchDemoButton({
  ...
  return (
    <div className="flex items-center justify-center p-2 sm:p-4 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-fit">
      <div className="text-center max-w-md py-1 sm:py-4">
        <div className="mb-2 sm:mb-4">
          <div className="inline-flex items-center justify-center w-12 sm:w-14 h-12 sm:h-14 rounded-full bg-blue-100 mb-2 sm:mb-3">
            <ExternalLink className="w-5 sm:w-6 h-5 sm:h-6 text-blue-600" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-1 sm:mb-2">{title}</h3>
          <p className="text-xs sm:text-sm text-gray-600 mb-2 sm:mb-4">{description}</p>
        </div>

        <motion.button
          onClick={handleLaunchDemo}
          disabled={isLoading}
          className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-semibold py-1 sm:py-2 px-4 sm:px-6 rounded-lg transition-all shadow-lg text-xs sm:text-sm"
        ...
        <div className="mt-2 sm:mt-4 p-2 sm:p-3 bg-white rounded-lg border border-blue-200">
